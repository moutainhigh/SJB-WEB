...
<head>
	...
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				//注意只有当type==3，也就是展示人员选择树弹窗时，才会启用异步加载机制
                async:{enable:(type==3),url:"${ctx}/sys/user/treeData?includeExEmployee=${includeExEmployee}",autoParam:["id=officeId","extId=extId"]},
                ...
                callback:{<%--
					...
					onClick:function(event, treeId, treeNode){
						...
						for (var i=0, l=nodes.length; i<l; i++) {
							...
							try{tree.checkNode(nodes[i], false, true);}catch(e){}
		...
		$(document).ready(function(){
			$.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&includeExEmployee=${includeExEmployee}&isAll=${isAll}&module=${module}&t="
					+ new Date().getTime(),
              function(zNodes){
				// 初始化树结构
				...
				for(var j=0; j<nodesOne.length; j++) {
					tree.reAsyncChildNodes(nodesOne[j],"!refresh",true);//强行异步加载父节点的子节点，参数"!refresh"表示追加子节点
				}
				selectCheckNode();
		...
		function selectCheckNode(){
			...
			for(var i=0; i<idArr.length; i++) {
				var node = tree.getNodeByParam("id", (type==3?"u_":"")+idArr[i]);
				if("${checked}" === "true"){
		...
		//更新节点状态
		function updateNodes(nodeList,nodes) {
			tree.showNodes(nodeList);
            var nodes = tree.transformToArray(nodes);
			for(var i=0, l=nodeList.length; i<l; i++) {

				for(var j=nodes.length-1; j>=0; j--) {
					if(nodes[j]["pId"] === nodeList[i]["id"]){
...
<body>
	...
	<div id="search" class="form-search hide" style="padding:10px 0 0 13px;">
		...
		<button class="btn" id="btn" onclick="searchNode()">搜索&nbsp;<i class="icon-search"></i></button>
	</div>
