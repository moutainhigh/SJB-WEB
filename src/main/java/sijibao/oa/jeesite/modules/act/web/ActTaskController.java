 ...
 */
package sijibao.oa.jeesite.modules.act.web;
...
public class ActTaskController extends BaseController {
	...
	public String todoList(TaskInfoEntity taskInfoEntity, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		taskInfoEntity.setAssignee(userId);
//		List<Act> list = actTaskService.todoList(act, userId);
//		model.addAttribute("list", list);
//		if (UserUtils.getPrincipal().isMobileLogin()){
//			return renderString(response, list);
//		}
		Page<TaskInfoEntity> page = actTaskService.queryTodoList(new Page<TaskInfoEntity>(request, response), taskInfoEntity);
		for(TaskInfoEntity t:page.getList()){
			t.setStatus("todo");
		}
		model.addAttribute("page", page);
		model.addAttribute(taskInfoEntity);
		model.addAttribute("officeId", taskInfoEntity.getOfficeId());
		model.addAttribute("officeName", taskInfoEntity.getOfficeName());
		if (UserUtils.getPrincipal().isMobileLogin()){
	...
	public String historicList(TaskInfoEntity taskInfoEntity, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		taskInfoEntity.setAssignee(userId);
//		Page<Act> page = new Page<Act>(request, response);
//		page = actTaskService.historicList(page, act, userId);
		Page<TaskInfoEntity> page = actTaskService.queryDoneList(new Page<TaskInfoEntity>(request, response), taskInfoEntity);
		
		//根据流程实例ID查询业务相关字段
		if(page.getList() != null && page.getList().size() > 0){
			for(TaskInfoEntity taskInfo:page.getList()){
				ExpenseFlow expenseFlow = expenseFlowService.getByProcInsId(taskInfo.getProcessInstanceId());
				DemandManagement demandManagement = demandManagementService.getDemandByProcInsId(taskInfo.getProcessInstanceId());
				RecpFlow recpFlow = recpFlowService.getByProcInsId(taskInfo.getProcessInstanceId());
				if(expenseFlow != null){
					taskInfo.setBusinessId(expenseFlow.getId());
				}else if(demandManagement != null){
					taskInfo.setBusinessId(demandManagement.getId());
				}else if(recpFlow != null){
					taskInfo.setBusinessId(recpFlow.getId());
				}
			}
		}
	...
	public String histoicFlow(Act act, String startAct, String endAct,String billType, Model model){
		if (StringUtils.isNotBlank(act.getProcInsId())){
			List<Act> histoicFlowList = new ArrayList<Act>();
			if(Constant.BILL_TYPE_DEMAND.equals(billType)){
				String executionId = "";
				String taskDefKey = "";
				if(StringUtils.isNotBlank(act.getExecutionId())){
					executionId = act.getExecutionId();
				}
				if(StringUtils.isNotBlank(act.getTaskDefKey())){
					taskDefKey = act.getTaskDefKey();
				}
				histoicFlowList = actTaskService.histoicFlowListForChildFlow(act.getProcInsId(), executionId,startAct, endAct,taskDefKey);
			}else{
				histoicFlowList = actTaskService.histoicFlowList(act.getProcInsId(), startAct, endAct);
			}
...
//		String formKey = "/flow/expenseFlow/form";
		// 获取流程实例对象
